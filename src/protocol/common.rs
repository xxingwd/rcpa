...
impl Operation {
    pub fn provider_protocol(self) -> ProviderProtocol {
        match self {
            Operation::Completions => ProviderProtocol::Completions,
            Operation::Responses => ProviderProtocol::Responses,
            Operation::Messages => ProviderProtocol::Messages,
            Operation::Embeddings => ProviderProtocol::Embeddings,
...
fn audit_request_headers(headers: &axum::http::HeaderMap) -> BTreeMap<String, String> {
...
pub async fn handle_llm_request(
    ...
    let requested_model = audit::model_from_body(&body);
    ...
    let candidate_protocols = translation::candidate_protocols(
        operation,
        body_value
            .get("stream")
    ...
    if !config_snapshot.has_endpoint_for_protocols(
        &model,
        Some(&auth_result.key),
        &candidate_protocols,
    ) {
        let err = AppError::ModelNotFound(model.clone());
        audit::record_llm_error(
            &state,
            request_id,
            api_key_id,
    ...
    let resolved_model = auth_result
        .key
        .model_aliases
        .get(&model)
        .cloned()
        .or_else(|| {
            config_snapshot
                .endpoints_for_alias(&model, None)
                .first()
                .map(|ep| ep.alias_name.clone())
        })
    ...
    let request_headers = audit_request_headers(headers);
...
mod tests {
    ...
    fn session_affinity_key_uses_api_key_id_not_secret() {
        ...
        assert_eq!(
            key.as_str(),
            "completions::completions::key-persisted-id::gpt-4o::session-123"
        );
