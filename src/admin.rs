...
fn check_admin(state: &AppState, headers: &HeaderMap) -> Result<(), AppError> {
    ...
    if token != state.config_service.snapshot().config.admin.token {
...
pub async fn create_provider(
    ...
    let max_connections = payload.max_connections;
    let timeout_secs = payload.timeout_secs;
    let headers = payload.headers.clone();
    let api_version = payload.api_version.clone();
    ...
    let snapshot = state
        ...
        .update(|config| {
            ...
            let provider = ProviderConfig {
                ...
                group: group.clone().unwrap_or_else(|| "default".to_string()),
            };
...
pub async fn vite_svg() -> impl IntoResponse {
