...
export default function LogsView({ showToast }) {
  ...
  const protocolOptions = useMemo(() => {
    const values = new Set(
      providersList.flatMap((provider) => (
        Array.isArray(provider.endpoints) ? provider.endpoints.map((endpoint) => endpoint?.protocol) : []
      )).filter(Boolean)
    );
    logs.forEach((log) => {
  ...
  const detailMetadata = useMemo(() => parseJsonString(detail?.meta), [detail?.meta]);
  const retryMetadata = detailMetadata && typeof detailMetadata === 'object' ? detailMetadata.retry : null;
  const retryAttempts = Array.isArray(retryMetadata?.attempts) ? retryMetadata.attempts : [];
  const requestMetadata = detailMetadata && typeof detailMetadata === 'object' ? detailMetadata.request : null;
  const routingMetadata = detailMetadata && typeof detailMetadata === 'object' ? detailMetadata.routing : null;
  const upstreamMetadata = detailMetadata && typeof detailMetadata === 'object' ? detailMetadata.upstream : null;
  const requestHeaders = requestMetadata?.headers && typeof requestMetadata.headers === 'object'
    ...
    <Card className="animate-in fade-in duration-500 h-full min-h-0 flex flex-col overflow-hidden">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Select value={filterModel} onValueChange={setFilterModel}>
              <SelectTrigger className="h-8 w-[180px] text-xs">
                <SelectValue placeholder="全部上游模型" />
              </SelectTrigger>
              <SelectContent>
      ...
      <CardContent className="flex-1 min-h-0 p-0">
        <div className="flex h-full min-h-0 flex-col">
          ...
          <Table className="min-w-[1440px] text-sm [&_th]:h-10 [&_th]:px-3 [&_td]:px-3 [&_td]:py-3">
            <TableHeader>
              <TableRow>
            ...
            <TableBody>
              {logs.length > 0 ? (
                logs.map((l, index) => {
                  ...
                  const formattedTime = formatLocalDateTime(l.created_at);
                  ...
                  return (
                    <TableRow key={l.id} className={index === 0 ? 'log-row' : ''}>
                      ...
                      <TableCell className="w-[8.5rem] max-w-[8.5rem] pl-1">
                      ...
                      <TableCell className="font-mono whitespace-nowrap">{l.retry_count ?? 0}</TableCell>
                      <TableCell>
                ...
                <TableRow>
                    <TableCell colSpan={18} className="text-center text-muted-foreground py-8">暂无接口请求记录</TableCell>
                </TableRow>
              )}
            </TableBody>
          </Table>
        ...
        {detailLoading ? (
          ...
          <div className="space-y-5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-xs">
              ...
              <div><span className="text-muted-foreground">时间：</span>{formatLocalDateTime(detail.created_at)}</div>
            </div>

            <div className="rounded-lg border bg-muted/20 p-3">
              <div className="text-[0.65rem] uppercase tracking-wider text-muted-foreground font-semibold mb-3">请求路由</div>
              <div className="grid grid-cols-1 gap-2 text-xs md:grid-cols-2">
                <div><span className="text-muted-foreground">请求协议：</span><span className="font-mono">{requestMetadata?.operation || detail.operation} / {requestMetadata?.protocol || detail.protocol}</span></div>
                <div><span className="text-muted-foreground">请求模型：</span><span className="font-mono break-all">{requestMetadata?.model || detail.model}</span></div>
                <div><span className="text-muted-foreground">解析模型：</span><span className="font-mono break-all">{requestMetadata?.resolved_model || detail.model}</span></div>
                <div><span className="text-muted-foreground">流式：</span><span className="font-mono">{String(Boolean(requestMetadata?.stream))}</span></div>
                <div><span className="text-muted-foreground">选中供应商：</span><span className="font-mono break-all">{routingMetadata?.selected_provider || detail.provider_name}</span></div>
                <div><span className="text-muted-foreground">选择原因：</span><span className="font-mono">{routingMetadata?.selected_provider_reason || '-'}</span></div>
                <div><span className="text-muted-foreground">目标协议：</span><span className="font-mono">{routingMetadata?.target_operation || upstreamMetadata?.operation || detail.operation} / {routingMetadata?.target_protocol || upstreamMetadata?.protocol || detail.protocol}</span></div>
                <div><span className="text-muted-foreground">目标模型：</span><span className="font-mono break-all">{routingMetadata?.target_model || upstreamMetadata?.model || '-'}</span></div>
                <div><span className="text-muted-foreground">Sticky：</span><span className="font-mono">{String(Boolean(routingMetadata?.sticky_hit))}</span></div>
                <div><span className="text-muted-foreground">上游状态：</span><span className="font-mono">{upstreamMetadata?.status_code ?? detail.status_code}</span></div>
                <div className="md:col-span-2"><span className="text-muted-foreground">上游地址：</span><span className="font-mono break-all">{upstreamMetadata?.base_url || routingMetadata?.upstream_base_url || '-'}</span></div>
              </div>
            </div>

            <div>
              <div className="text-[0.65rem] uppercase tracking-wider text-muted-foreground font-semibold mb-2">请求 Headers</div>
              <CodeBlock className="max-h-[260px]">{formatJson(requestHeaders) || '{}'}</CodeBlock>
            </div>
            ...
            <div className="rounded-lg border bg-muted/20 p-3">
              <div className="mb-3 flex items-center justify-between gap-3">
                <div className="text-[0.65rem] uppercase tracking-wider text-muted-foreground font-semibold">重试</div>
                <div className="flex flex-wrap items-center gap-2 text-xs">
                  <Badge variant="secondary" className="font-mono">retry={retryMetadata?.retry_count ?? 0}</Badge>
                  <Badge variant="outline" className="font-mono">attempts={retryMetadata?.attempt_count ?? 0}</Badge>
                  <Badge variant="outline" className="font-mono">backoff={formatDuration(retryMetadata?.total_backoff_ms ?? 0)}</Badge>
                </div>
              </div>
              {retryAttempts.length > 0 ? (
                <div className="space-y-2">
                  {retryAttempts.map((attempt) => (
                    <div key={`${attempt.attempt}-${attempt.provider_name}-${attempt.status_code}`} className="rounded-md border bg-background/80 p-2.5">
                      <div className="flex flex-wrap items-center gap-2 text-xs">
                        <Badge variant="outline" className="font-mono">#{attempt.attempt}</Badge>
                        <Badge variant={attempt.retryable ? 'destructive' : 'success'} className="font-mono">
                          {attempt.status_code}
                        </Badge>
                        <span className="font-mono">{attempt.provider_name}</span>
                        <span className="text-muted-foreground">{attempt.protocol}</span>
                        <span className="text-muted-foreground">selected={attempt.selected_via}</span>
                        <span className="text-muted-foreground">sticky={String(attempt.sticky_hit)}</span>
                        <span className="text-muted-foreground">healthy={String(attempt.provider_healthy_before_attempt)}</span>
                        <span className="text-muted-foreground">backoff={formatDuration(attempt.backoff_ms_before_next ?? 0)}</span>
                      </div>
                      <div className="mt-1 text-xs text-muted-foreground break-all">
                        {attempt.error_code || 'ok'}{attempt.error_message ? `: ${attempt.error_message}` : ''}
                      </div>
                    </div>
                  ))}
                </div>
              ) : (
                <div className="text-xs text-muted-foreground">无重试</div>
              )}
            </div>

            <div>
              <div className="text-[0.65rem] uppercase tracking-wider text-muted-foreground font-semibold mb-2">元数据 JSON</div>
              <CodeBlock className="max-h-[420px]">{formatJson(detailMetadata) || '{}'}</CodeBlock>
            </div>

            <div>
              <div className="text-[0.65rem] uppercase tracking-wider text-muted-foreground font-semibold mb-2">上游请求 JSON</div>
