...
export default function KeysView({ showToast }) {
  ...
  const openCreateModal = async () => {
    await Promise.all([fetchModelCatalog(), fetchProviders()]);
    setEditingKey(null);
    setName('');
    setLabels('');
    setAllowedProviders([]);
  ...
  const openEditModal = async (key) => {
    ...
    setAllowedProviders(Array.isArray(key.allowed_providers) ? key.allowed_providers : []);
    setModelRows(hydrateModelRows(key));
    setIsModalOpen(true);
  };

  const toggleAllowedProvider = (providerName, checked) => {
    setAllowedProviders((current) => {
      ...
      return providers
        ...
        .filter((nameValue) => next.has(nameValue));
    });
  };
          ...
          <TableBody>
            {keys.length > 0 ? (
              keys.map((key) => {
                ...
                const providerRules = Array.isArray(key.allowed_providers) ? key.allowed_providers : [];
                  ...
                  <TableRow key={key.id}>
                    ...
                    <TableCell className="max-w-[180px]">
                      {providerRules.length === 0 ? (
                        ...
                        <div className="flex flex-wrap gap-1">
                          {providerRules.map((provider) => (
                            <Badge key={provider} variant="secondary" className="max-w-full font-mono text-xs">
                              {provider}
                            </Badge>
                          ))}
                        </div>
              ...
              <TableRow>
                <TableCell colSpan={8} className="text-center text-muted-foreground py-8">未找到 API 密钥</TableCell>
              </TableRow>
            )}
          </TableBody>
        </Table>
      ...
      <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
        <DialogContent className="max-w-[760px]">
          ...
          <form onSubmit={handleSaveKey} className="space-y-4">
            ...
            <div className="space-y-2">
              <Label className="text-xs uppercase tracking-wider text-muted-foreground">允许供应商</Label>
              <div className="rounded-lg border border-border bg-muted/30 p-3">
                {providers.length > 0 ? (
                  <div className="grid gap-2 sm:grid-cols-2">
                    {providers.map((provider) => {
                      const checked = allowedProviders.includes(provider.name);
                      return (
                        <label
                          key={provider.name}
                        ...
                        >
                          <Checkbox
                            checked={checked}
                            onCheckedChange={(value) => toggleAllowedProvider(provider.name, Boolean(value))}
                            className="mt-0.5"
                          />
                          <div className="min-w-0">
                            <div className="font-mono text-sm">{provider.name}</div>
                            <div className="text-[11px] text-muted-foreground">
                              {(provider.endpoints || []).map((endpoint) => endpoint?.protocol).filter(Boolean).join(', ') || '未声明协议'}
                            </div>
                          </div>
                        </label>
                      );
                  ...
                  <div className="text-xs text-muted-foreground">暂无可选供应商</div>
                )}
